...
You must do a one-time scan of your OpenRCT rides using the Smart Scan button (and do it again if you install new content), then just pick how many total technologies you want available over the course of the game (anywhere from 1x, which is unchanged from the default numbers, up to 3x, which triples the total list of technolgies you can unlock unless you run out of new rides to research!) and select if you want to be guaranteed to start with the Cash Machine or Info Kiosk unlocked.
